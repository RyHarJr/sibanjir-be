...
function scoreRoute(
  coords: number[][],
  floodReports: Array<{ latitude: number; longitude: number; waterDepthCm: number; severityLevel: string; roadAccess: string | null; address: string | null }>
) {
  let floodScore = 0;
  let isBlocked = false;
...
async function fetchOSRM(
  ...
  const url =
    `${OSRM_BASE}/route/v1/driving/${coordinateStr}` +
    `?alternatives=${waypoints.length === 0 ? "true" : "false"}` +
...
function generateMultiDetourCandidates(
  originLat: number, originLng: number,
  destLat: number,   destLng: number,
  floodReports: Array<{ latitude: number; longitude: number; waterDepthCm: number }>
  ...
  const clusterSpreadKm = Math.max(
    ...blockers.map(r => haversine(centLat, centLng, r.latitude, r.longitude)),
    0.1
  );
  ...
  const t = Math.max(0.15, Math.min(0.85,
    ((centLat - originLat) * dLat + (centLng - originLng) * dLng) / (len * len)
  ));
...
router.post("/safe-route", async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const scoredRoutes = osrmData.routes.map((route: any, index: number) => {
      ...
      const summary =
        route.legs?.map((l: any) => l.summary || "").filter(Boolean).join(", ") || "";
      ...
      const detourResults = await Promise.allSettled(
        detourCandidates.map(async (waypoints) => {
          ...
          return {
            ...
            summary: detourRoute.legs
              ?.map((l: any) => l.summary || "").filter(Boolean).join(", ") || "Rute Detour",
            index: -1,
          };
        })
      );
    ...
    const floodedRoute =
      scoredRoutes.length > 1 && scoredRoutes[scoredRoutes.length - 1].floodScore > safeRoute.floodScore
        ? scoredRoutes[scoredRoutes.length - 1]
        : null;
    ...
    const allFloodMarkers = floodReports
      .filter((r) =>
        routePoints.some(
          (p) => haversine(p.lat, p.lng, r.latitude, r.longitude) < 1
        )
      )
      .map((r) => ({
        lat: r.latitude,
