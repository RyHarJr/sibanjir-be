...
// ── GET /reports ───────────────────────────────────────────────────────────
router.get("/", [query("status").optional().isIn(["active", "surging", "receded"]), query("severity").optional().isIn(["low", "medium", "high", "extreme"]), query("district").optional().isString(), query("sort").optional().isIn(["latest", "deepest", "confidence"]), query("userId").optional().isInt(), query("page").optional().isInt({ min: 1 }), query("dateFrom").optional().isString(), query("dateTo").optional().isString()], validate, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const orderBy: Record<string, string> = sort === "deepest" ? { waterDepthCm: "desc" } : sort === "confidence" ? { confidenceScore: "desc" } : { createdAt: "desc" }
...
// ── POST /reports ──────────────────────────────────────────────────────────
router.post("/", authenticate, [body("title").trim().notEmpty().withMessage("Judul wajib diisi"), body("description").trim().notEmpty().withMessage("Deskripsi wajib diisi"), body("latitude").isFloat({ min: -90, max: 90 }).withMessage("Latitude tidak valid"), body("longitude").isFloat({ min: -180, max: 180 }).withMessage("Longitude tidak valid"), body("waterDepthCm").isInt({ min: 0 }).withMessage("Tinggi air harus angka positif"), body("roadAccess").isIn(["passable", "motorcycle_only", "difficult", "impassable"]), body("waterCurrent").optional().isIn(["calm", "slow", "moderate", "fast"]), body("districtId").optional().isInt(), body("address").optional().isString()], validate, async (req: AuthRequest, res: Response): Promise<void> => {
...
router.post("/:id/update", authenticate, [body("waterDepthCm").isInt({ min: 0 }), body("status").isIn(["active", "surging", "receded"]), body("description").optional().isString()], validate, async (req: AuthRequest, res: Response): Promise<void> => {
...
router.post("/:id/verify", authenticate, [body("vote").isIn(["confirm", "reject"]).withMessage("Vote harus confirm atau reject"), body("comment").optional().isString()], validate, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const [confirms, rejects] = await Promise.all([prisma.reportVerification.count({ where: { reportId, vote: "confirm" } }), prisma.reportVerification.count({ where: { reportId, vote: "reject" } })])
...
router.post("/:id/admin-verify", authenticate, requireAdmin, [body("action").isIn(["verify", "reject"]).withMessage("Action harus verify atau reject")], validate, async (req: AuthRequest, res: Response): Promise<void> => {
  const reportId = Number(req.params.id)
...
router.post("/:id/photos", authenticate, upload.array("photos", 10), async (req: AuthRequest, res: Response): Promise<void> => {
  const reportId = Number(req.params.id)
